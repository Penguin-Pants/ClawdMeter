...
def _run(coro):
    ...
    Python 3.12+ removed the implicit "current event loop" — get_event_loop()
    no longer auto-creates one. asyncio.run() works on all supported versions
    (it manages its own loop lifecycle).
    """
...
def test_poll_api_returns_none_on_4xx_without_ratelimit_headers(monkeypatch):
    """poll_api returns None on a 4xx with NO rate-limit headers (genuine transient)."""
...
def test_poll_api_returns_none_not_autherror_on_429(monkeypatch):
    """Rate-limit (429) WITHOUT rate-limit headers is transient — None, NOT
    AuthError (regression guard for the 401/403-vs-other-4xx split)."""
    mock_resp = _make_mock_response(status_code=429, headers={})
    mock_client = AsyncMock()
    mock_client.__aenter__ = AsyncMock(return_value=mock_client)
    mock_client.__aexit__ = AsyncMock(return_value=False)
...
def test_poll_api_returns_payload_on_429_with_ratelimit_headers(monkeypatch):
    """A 429 from a fully-exhausted 5h window still carries the same
    anthropic-ratelimit-* headers as a 200. The daemon must surface those to
    the firmware (s=100, sr=<minutes-to-reset>) so it can show the
    'Usage Limit Reached' screen instead of going stale on 'No data'."""
    now = time.time()
    mock_resp = _make_mock_response(
        status_code=429,
        headers={
            "anthropic-ratelimit-unified-5h-utilization": "1.0",
            "anthropic-ratelimit-unified-5h-reset": str(now + 8040),  # 134 minutes
    ...
    assert payload is not None, "429 with rate-limit headers must yield a payload"
    assert payload["s"] == 100
    assert payload["w"] == 50
    assert payload["st"] == "limited"
    assert payload["ok"] is True
    assert abs(payload["sr"] - 134) <= 1, f"Expected ~134, got {payload['sr']}"
