...
standing in for the panel — `main.cpp`, `ui.cpp`, `splash.cpp`, idle fade, pair
gesture, and JSON parsing all run unmodified. Only `ble.cpp` is swapped for a
scenario-playback stub. Sources live in `firmware/src/boards/sim/` (HAL against
SDL2 + Arduino shims in `shim/`), with scenario data in `firmware/sim/`.
